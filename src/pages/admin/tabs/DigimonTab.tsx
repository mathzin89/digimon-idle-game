...
interface DigimonData { id: string; name: string; hp: number; atk: number; evolvesTo: string; evolveLevel: number; rarity: string; menuImg: string; }
...
export function DigimonTab() {
  ...
  const [formData, setFormData] = useState<DigimonData>({ id: '', name: '', hp: 1000, atk: 10, evolvesTo: '', evolveLevel: 0, rarity: 'Normal', menuImg: '' });
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex justify-between items-center bg-[#111827] p-5 rounded-lg border border-[#1e293b] shadow-sm">
        <div>
          ...
          <p className="text-[10px] text-slate-500 uppercase tracking-widest">Cadastre atributos, evoluções e links de sprites no Firebase.</p>
        </div>
        <button onClick={() => { setIsEditing(false); setFormData({ id: '', name: '', hp: 1000, atk: 10, evolvesTo: '', evolveLevel: 0, rarity: 'Normal', menuImg: '' }); setIsModalOpen(true); }} className="bg-cyan-900/30 border border-cyan-500 hover:bg-cyan-800/50 text-cyan-400 px-5 py-2.5 rounded-md text-[10px] font-bold uppercase tracking-widest transition-colors flex items-center gap-2">
          <span className="text-lg leading-none">+</span> Novo Digimon
        </button>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#111827] border-b border-[#1e293b] text-[9px] text-slate-400 uppercase tracking-widest">
              <th className="p-4 font-bold">ID / Sprite</th>
              ...
              digimons.map((digi) => (
                <tr key={digi.id} className="border-b border-[#1e293b] hover:bg-[#111827]/50 transition-colors">
                  <td className="p-4 flex items-center gap-3">
                    <div className="w-10 h-10 bg-black rounded-full border border-[#1e293b] flex items-center justify-center shadow-inner overflow-hidden">
                       {digi.menuImg ? <div className="pixelated w-full h-full" style={{ backgroundImage: `url('${digi.menuImg}')`, backgroundSize: '300% 100%', backgroundPosition: '0% 0%' }} /> : <span className="text-[10px]">❌</span>}
                    </div>
            ...
            <form onSubmit={handleSave} className="p-6 flex-1 overflow-y-auto custom-scrollbar space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.evolvesTo} onChange={e => setFormData({...formData, evolvesTo: e.target.value})} className="w-full bg-[#111827] border border-[#1e293b] focus:border-yellow-400/50 rounded px-3 py-2 text-yellow-400 text-xs outline-none font-mono" placeholder="Vazio = Final" />
                </div>
                <div>
              ...
              <div>
                <label className="block text-slate-400 text-[9px] font-bold uppercase tracking-widest mb-1">Caminho da Sprite</label>
                <input type="text" required value={formData.menuImg} onChange={e => setFormData({...formData, menuImg: e.target.value})} className="w-full bg-[#111827] border border-[#1e293b] focus:border-cyan-400 rounded px-3 py-2 text-white text-xs outline-none font-mono" placeholder="/sprite.png" />
              </div>
              <div className="pt-4 border-t border-[#1e293b] flex justify-end gap-3">
                ...
                <button type="submit" className="bg-cyan-900/40 border border-cyan-500 text-cyan-400 hover:bg-cyan-800/60 px-6 py-2 rounded text-[10px] font-bold uppercase tracking-widest transition-colors shadow-[0_0_15px_rgba(34,211,238,0.2)]">Salvar no Banco</button>
              </div>
            </form>
          </div>
        </div>
