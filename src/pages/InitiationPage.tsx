...
const STARTERS = [
  { id: 'agumon', name: 'Agumon', type: 'Fogo', img: 'https://wikimon.net/images/4/4c/Agumon_b_ds.gif', desc: 'Ataque alto. Perfeito para combates agressivos.' },
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center relative overflow-hidden font-sans selection:bg-digi-cyan/30">
      ...
      <div className="relative z-10 w-full max-w-4xl p-8 flex flex-col items-center min-h-[500px]">
        ...
        {/* PASSO 2: ESCOLHER GÊNERO COM O SPRITE RECORTADO */}
        {step === 2 && (
          <div className="w-full animate-in slide-in-from-right duration-500 flex flex-col items-center">
            ...
            <div className="grid grid-cols-2 gap-8 w-full max-w-md mb-12">
              ...
              >
                <div className="w-20 h-20 bg-slate-950 rounded-lg flex items-center justify-center mb-4 border border-slate-700 scale-150">
                  <TamerSprite gender="male" direction="down" />
                </div>
              ...
              >
                <div className="w-20 h-20 bg-slate-950 rounded-lg flex items-center justify-center mb-4 border border-slate-700 scale-150">
                  <TamerSprite gender="female" direction="down" />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-12">
              {STARTERS.map((st) => (
                <div 
                  ...
                  className={`bg-slate-900/50 border-2 rounded-xl p-6 flex flex-col items-center text-center cursor-pointer transition-all ${selectedStarter === st.id ? 'border-emerald-400 shadow-[0_0_30px_rgba(52,211,153,0.3)] scale-105' : 'border-slate-800 hover:border-slate-600'}`}
                >
                  <img src={st.img} alt={st.name} className="h-20 object-contain pixelated mb-4 drop-shadow-[0_10px_10px_rgba(0,0,0,0.8)]" />
                  <h3 className={`font-black tracking-widest uppercase text-lg mb-1 ${selectedStarter === st.id ? 'text-emerald-400' : 'text-slate-300'}`}>{st.name}</h3>
